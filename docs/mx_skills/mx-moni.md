...
> **已实测的市场识别限制（2026-08）**：
> - ✅ 沪市股票（6 开头，如 `600519`）
> - ✅ 深市股票（0/3 开头，如 `000001`、`300750`）
> - ✅ 沪市 ETF（5 开头，如 `513100`、`510300`、`518880`）
> - ✅ 北交所（9 开头）
> - ❌ **深市 ETF / LOF（1 开头，如 `159938`、`159934`、`161725`）一律返回"无法判断股票市场号"**，即使补 `.SZ` 后缀、传 `secMkt` 字段也无法绕过。
>
> 因此：**妙想模拟仓无法交易任何 1 开头的深市 ETF/LOF**。配置 ETF 组合时需使用沪市等价标的。
