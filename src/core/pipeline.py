...
class StockAnalysisPipeline:
    ...
    def run(
        ...
        fail_count = len(stock_codes) - len(results)
        ...
        if results:
            try:
                from src.services.trend_pool_service import TrendPoolService

                trend_pool = TrendPoolService()
                stats = trend_pool.update_pool(results)
                logger.info(
                    f"📈 趋势跟踪池更新 | 新增:{len(stats.get('new_entries', []))} "
                    f"继续:{len(stats.get('continued', []))} 移除:{len(stats.get('removed', []))} "
                    f"总计:{stats.get('total_in_pool', 0)}"
                )

                # 生成并保存跟踪池日报
                pool_report = trend_pool.generate_daily_report()
                pool_report_path = self._save_trend_pool_report(pool_report)
                logger.info(f"📊 趋势跟踪池日报已保存: {pool_report_path}")

            except Exception as e:
                logger.warning(f"趋势跟踪池更新失败: {e}")

        return results

    def _save_trend_pool_report(self, report: str) -> str:
        """保存趋势跟踪池报告到本地文件"""
        ...
        filename = f"trend_pool_{today_str}.md"
