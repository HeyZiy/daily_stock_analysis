...
### 8. 流动池选股梯队（重点关注）
- **梯队 1**: 命中"缩量回踩，站稳五日线"形态。
- **形态特征**: 今日量比萎缩，最低价回测 MA5 支撑，且收盘价守住 MA5。
- **分析策略**: 此类股票属于池内重点关注的目标，代表回踩企稳，具有较高的博弈性价比，应在结论中显著点出。
...
- **明天可买入信号**（高优先级）：
  - 乖离率 < 2%：最佳买点，明天可直接买入
  - 缩量回踩 MA5：洗盘特征，趋势延续概率高
  
- **继续观望信号**（中优先级）：
  - 乖离率 2-5%：等待回踩 MA5 后再买入
  - 趋势良好但位置偏高，需耐心等待
...
```json
{
    ...
    "trend_pool": {
        "eligible": true/false,           // 是否符合入池条件
        "priority": "high/medium/low",    // 优先级：high=明天可买入, medium=继续观望, low=等待回调
        "suggested_action": "立即买入/明天买入/继续观望/等待回调/不入池",
        "track_reason": "入池理由或不符合条件的原因"
    }
}
```
