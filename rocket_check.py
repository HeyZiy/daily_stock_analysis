...
对单只 ETF 跑卫星仓火箭规则（口径与 src/etf/rocket_breakout.py 完全一致），
只输出诊断，不拉持仓、不下单。

用法：
  python rocket_check.py 512400                  # 有色金属ETF南方（新浪日线）
  python rocket_check.py 512400 --name 有色金属   # 显式指定名称
...
from src.etf import rocket_breakout as rb
from data_provider import get_etf_daily, is_etf_code

logger = logging.getLogger(__name__)

# 指数/个股不在诊断范围；rocket 仅诊断 ETF（见 main 中 is_etf_code 校验）。
...
def _diagnosis(res: dict, is_index: bool, meta: dict, days: int) -> str:
    ...
    chg_ok = rb.BREAKOUT_CHANGE_MIN <= chg <= rb.BREAKOUT_CHANGE_MAX
    vol5_ok = res["vol_ratio_5"] >= rb.VOL_MULT_5D
    vol20_ok = res["vol_ratio_20"] >= rb.VOL_MULT_20D

    L = ["=" * 56]
    src = "指数口径 · 中证官网" if is_index else "ETF口径 · 新浪日线"
    L.append(f"🚀 火箭单只诊断 — {res['name']}({res['code']})  [{src}]")
    L.append("=" * 56)
    L.append(f"最新数据日 {res['df']['date'].iloc[-1]} | 收盘 {res['close']:,.2f} | 当日涨幅 {chg:+.2f}%")
    L.append("")
    L.append(f"{_mark(price_ok)} 价能  {shape} | 实体占比 {res['body_ratio']:.2f} | 20日涨幅 {res['ret_20d']:+.1f}%")
    L.append(f"{_mark(chg_ok)} 涨幅  {chg:+.2f}%（需 {rb.BREAKOUT_CHANGE_MIN:.1f}% ~ {rb.BREAKOUT_CHANGE_MAX:.1f}%）")
    L.append(f"{_mark(vol5_ok)} 量能  量比5日 {res['vol_ratio_5']:.2f}（需 ≥{rb.VOL_MULT_5D:.1f}）")
    L.append(f"{_mark(vol20_ok)} 量能  量比20日 {res['vol_ratio_20']:.2f}（需 ≥{rb.VOL_MULT_20D:.1f}）")
    if res["pe_pct"] is not None:
        L.append(f"{_mark(res['pe_pct'] < rb.PE_LOCK)} 估值  行业PE分位 {res['pe_pct']:.0f}%（≥{rb.PE_LOCK:.0f}% 锁仓）")
    else:
        pe_extra = f" | 中证滚动PE {meta['pe']:.1f}" if meta.get("pe") else ""
        L.append(f"⬜ 估值  无行业PE分位映射（不硬剔，仅少评分）{pe_extra}")
    L.append("")
    L.append(f"火箭评分 {res['rocket_score']}（入围线 {rb.SCORE_THRESHOLD}）| 动量观察分 {res['rot_score']}")
    verdict = "🔥 触发放量突破" if res["breakout"] else "未触发放量突破"
    if res["breakout"] and res["rocket_score"] < rb.SCORE_THRESHOLD:
        verdict += "（但评分未达入围线，实战不入选）"
    L.append(f"判定: {verdict}")
    L.append("")
    L.append(f"近{days}日明细:")
    L.extend(_recent_rows(res, days))
    L.append("")
    L.append("*诊断口径与卫星仓引擎一致；正式买卖由 etf_observe.py 周度批次决定。")
    return "\n".join(L)


def main():
    parser = argparse.ArgumentParser(description="ETF 火箭单只诊断（口径同卫星仓引擎，只诊断不交易）")
    ...
    setup_logging(log_prefix="rocket_check", debug=args.debug)

    code = args.code.strip()
    if not is_etf_code(code):
        print(f"{code} 不是 ETF。rocket 只诊断卫星仓 ETF 标的（如 512400 有色金属ETF南方）。")
    ...
    res = rb.analyze_etf(code, name, df=injected_df)
    if res is None:
